...
function template_main()
{
	...
	{
		echo '
	<div id="board_', $context['current_board'], '_childboards" class="boardindex_table p-2 space-y-2 bg-white rounded-lg shadow">
		<div class="items-center px-4 py-2 space-x-4 text-white rounded-lg shadow bg-gradient-to-r from-purple-700 to-purple-600 ">
	...
	if (!empty($board['children']))
	{
		...
		foreach ($board['children'] as $child)
		{
			if (!$child['is_redirect'])
				$child['link'] = '' . ($child['new'] ? '<i class="fas fa-circle" style="color:orange"></i> ' : '') . '<a href="' . $child['href'] . '" ' . ($child['new'] ? 'class="board_new_posts1" ' : '') . 'title="' . ($child['new'] ? $txt['new_posts'] : $txt['old_posts']) . ' (' . $txt['board_topics'] . ': ' . comma_format($child['topics']) . ', ' . $txt['posts'] . ': ' . comma_format($child['posts']) . ')">' . $child['name'] . '</a>';
			else
			...
			$children[] = $child['new'] ? '<span class="strong flex items-center space-x-2"> ' . $child['link'] . '</span>' : '<span class="flex items-center space-x-2"><i class="fas fa-circle" style="color:#bcbdc3;"></i> ' . $child['link'] . '</span>'; 
		}
